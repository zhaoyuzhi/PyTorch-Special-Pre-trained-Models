...
def Trainer(opt):
    ...
    if opt.pre_train == True:
        net = network_Gray_ResNet.resnet50()
    ...
    def save_model(net, epoch, opt):
        ...
        if opt.multi_gpu == True:
            if epoch % opt.checkpoint_interval == 0:
                torch.save(net.module, '%s_BN_epoch%d_batchsize%d.pth' % (opt.load_type, (epoch + addition_epoch), opt.batch_size))
                print('The trained model is successfully saved at epoch %d' % epoch)
        else:
            if epoch % opt.checkpoint_interval == 0:
                torch.save(net, '%s_BN_epoch%d_batchsize%d.pth' % (opt.load_type, (epoch + addition_epoch), opt.batch_size))
                print('The trained model is successfully saved at epoch %d' % epoch)
